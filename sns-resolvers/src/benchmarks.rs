//! Benchmarking setup for sns-pallets
...
benchmarks! {
    where_clause {
        where
        T: sns_registrar::origin::Config + sns_registrar::registrar::Config,
...
where
    T: sns_registrar::registrar::Config + sns_registrar::origin::Config,
{
    let owner = create_caller::<T, T::Currency>(888);
    let owner_clone = owner.clone();
    sns_registrar::registrar::Pallet::<T>::register(
    ...
    Ok((
        owner_clone,
        sns_registrar::traits::Label::<T::Hash>::new("cupnfishuuu".as_bytes())
            .unwrap()
            .0
