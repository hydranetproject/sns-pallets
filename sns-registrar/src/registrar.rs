...
//! 1. the deposit is the transaction of the registered domain name to the `SnsOfficial` account
//! 2. the `SnsOfficial` account then saves the deposit through `T::Currency::reserve` so that it cannot be withdrawn.
//! 3. when the domain is `reclaimed` by the user, `SnsOfficial` calls `T::Currency::unreserve` and returns the deposit to the caller. (Or if the domain name expires and is registered by someone else, then the deposit return logic is executed)
//!
...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn register(
            ...
            // Prevent calculation result from overflowing
            ensure!(
