	...
	impl<T: Config> Pallet<T> {
		...
		fn handle_bid(bidder: T::AccountId, node: T::Hash, amount: BalanceOf<T>) -> DispatchResult {
			...
			if current_winning.map_or(true, |last| amount > last.1) {
				...
				// todo：I don't know if it's useful here
				...
				// 	// returns the funds of inactive users by judging whether the state of the auction is not over
				// 	// This function is necessary, but it needs to be optimized to determine which are not active bidders
		...
		fn check_auction_end(now: T::BlockNumber, node: T::Hash) -> Option<WinnerData<T>> {
			AuctionInfo::<T>::get(node).and_then(|start_time| {
				...
				if is_ended {
					let (raw_offset, known_since) = T::Randomness::random(&b"sns_auction"[..]);
		...
		fn manage_auction_end(node: T::Hash, winner_data: WinnerData<T>) -> DispatchResult {
			// Release previously seized currency
