...
use mock::*;
use sns_resolvers::resolvers::{Address, TextKind};
...
fn register_test() {
    new_test_ext().execute_with(|| {
        ...
        assert!(len2 == 11);
        let node = label.encode_with_node(&SEL_BASENODE);
        let node2 = label2.encode_with_node(&SEL_BASENODE);
...
fn redeem_code_test() {
    new_test_ext().execute_with(|| {
        ...
        let test_node = label.encode_with_node(&SEL_BASENODE);
...
fn resolvers_test() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            ...
            sns_resolvers::resolvers::Error::<Test>::InvalidPermission
        );
    })
}
